...
class LyricsViewState(
    ...
    suspend fun play(currentTime: Long) {

        val lines = lyrics?.lines ?: return


        val index = getNearLine(currentTime, lines)
        if (index == -1) {
            return
        }

        seekToLine(index)
        delay(lines[index].startAt - currentTime)
        play()
    }

    private fun getNearLine(currentTime: Long, lines: List<Lyrics.Line>): Int {

        lines.forEachIndexed { index, line ->
            if (line.startAt > currentTime) {
                return index
            }
        }
